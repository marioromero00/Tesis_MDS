...
$renderDir = Join-Path $env:TEMP 'mds-pptx-tema-23-09-2026'
...
try {
    ...
    $audit = [pscustomobject]@{opened_in='Microsoft PowerPoint'; slides=$deck.Slides.Count; overflow=@($overflow); tolerance_points=2; pdf_pages_expected=7; canva_import_tested=$false}
    $reportPath = Join-Path $repoDir 'documentacion/Verificacion_PPTX_Tema_23-09-2026.json'
    ...
    Copy-Item -LiteralPath (Join-Path $renderDir 'presentacion.pdf') -Destination ([System.IO.Path]::ChangeExtension($pptPath, '.pdf')) -Force
} finally {
