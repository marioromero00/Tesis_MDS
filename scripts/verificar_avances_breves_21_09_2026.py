...
HTML = ROOT / 'presentaciones/Avances-Resumen-Autocontenido-21-09-2026.html'
TEMP = Path(os.environ['TEMP']) / 'mds-presentation-21-09-2026'
...
text_html = HTML.read_text(encoding='utf-8')
with (ROOT/'resultados/grids_recurrentes_13-09-2026/resumen.csv').open() as f:
    for row in csv.DictReader(f):
        if row['family'] == row['scope']:
            check('BA '+row['family'], f"{float(row['ba_macro']):.4f}".replace('.',',') in text_html)
with (ROOT/'resultados/ablacion_pca_12-09-2026/resumen_ablaciones.csv').open() as f:
    for row in csv.DictReader(f):
        if row['variant'] in ['reference','without_eeg','drop_fixation_count','pca_0.9']:
            digits=5 if row['variant']=='reference' else 4
            check('BA '+row['variant'], format(float(row['macro_score']),f'.{digits}f').replace('.',',') in text_html)
...
with sync_playwright() as p:
    ...
    check('Cinco diapositivas, una visible', page.locator('.slide').count() == 5 and page.locator('.slide:visible').count() == 1)
    page.keyboard.press('ArrowRight')
    check('Flecha siguiente', page.locator('#status').inner_text() == '2 / 5')
    page.keyboard.press('End')
    check('Fin', page.locator('#s5').is_visible())
    ...
    check('Lectura continua', page.locator('.slide:visible').count() == 5)
    ...
    check('Impresion de cinco diapositivas', page.locator('.slide:visible').count() == 5)
    ...
    check('PDF cinco paginas completas', len(doc) == 5 and all(len(x.get_text())>200 for x in doc))
    ...
    check('Espacio activa lectura', page.locator('.slide:visible').count() == 5)
    page.keyboard.press('Space')
    ...
    check('Contenido completo sin JavaScript', nojs.locator('.slide:visible').count() == 5)
    browser.close()
...
(ROOT/'documentacion/Verificacion_Avances_Breves_21-09-2026.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
