...
from transformer_grid_core import grid,train,predict
...
OUT=ROOT/'resultados/transformer_grid_13-09-2026'
FRAME=None
...
def main():
    ...
    sources=['transformer_grid_core.py','entrenar_transformer_grid.py','redes_secuencia_13.py',
    ...
    if args.resume:
        ...
        protocol=dict(created_utc=datetime.now(timezone.utc).isoformat(),grid=grid(),folds=folds,
            ...
            primary='grid-selected Transformer versus previous Transformer and Ridge',
            ...
            initialization='independent layers; prediction and checkpoint callback preserve torch RNG; earlier source left intact',
    ...
    tasks=[dict(kind='outer',fold=folds[int(c['fold'])-1],config=next(g for g in grid() if g['id']==c['id']),
           ...
           for c in choices for scope in ['Transformer','repeat_current'] for seed in protocol['seeds']]
    results=execute(tasks)
